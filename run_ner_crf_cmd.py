...
name = {"address":31, "book":32, "company":33, 'game':34, 'government':35, 'movie':36, 'name':41,
        'organization':42, 'position':43,'scene':44}
...
def predict(args, model, tokenizer, message,output, prefix=""):
    pred_output_dir = args.output_dir
    if not os.path.exists(pred_output_dir) and args.local_rank in [-1, 0]:
        os.makedirs(pred_output_dir)
    test_dataset = load_and_cache_examples_ours(args, args.task_name, tokenizer, message,data_type='test')
    # Note that DistributedSampler samples randomly
...
def load_and_cache_examples_ours(args, task, tokenizer, message,data_type='train'):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    processor = processors[task]()
    # Load data features from cache or dataset file
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            for name_key in name:
                if name_key != "scene":
                    print('\033[1;%dm%s\033[0m'%(name[name_key],name_key))
                else:
                    print('\033[1;%dm%s\033[0m'%(name[name_key],name_key),end="")
            while active:
                prompt = "\n请输入将要抽取的文本:"
